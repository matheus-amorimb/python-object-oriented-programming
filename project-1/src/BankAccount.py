...
class BankAccount():
    ...
    def __init__(self, id, first_name, last_name, timezone = None, initial_balance = 0):
    ...
    def is_string(value):
        if not isinstance(value, str):
            raise ValueError(f'Name must be passed as string. It was passed a {type(value).__name__}.')
        return value
    ...
    def check_name(value, field_name):
        ...
        if not value.isalpha():
            raise ValueError(f'{field_name} must contain only letters and cannot be empty.')
        return value
    ...
    def parse_confirmation_code(confirmation_code, preferred_time_zone=None):
        Confirmation = namedtuple('Confirmation', ['transaction_code', 'account_number', 'time', 'time_utc', 'transaction_id'])
        ...
        transaction_code, account_number, time, timezone, transaction_id = parts
        ...
        time_utc = datetime.strptime(time, '%Y%m%d%H%M') - preferred_time_zone.offset
        time_utc_str = time_utc.strftime('%Y-%m-%d %H:%M:%S')
        
        return Confirmation(transaction_code, account_number, preferred_time_str, time_utc_str, transaction_id)
    ...
    def apply_interest(self):
        ...
        conf_code = self.generate_confirmation_code(BankAccount._transaction_codes['interest'])
